...
export function DealAttachments({ workspaceId, dealId }: DealAttachmentsProps) {
    const { dealAttachments, fetchDealAttachments, uploadDealAttachment, deleteDealAttachment } = useCrmStore();
    const fileInputRef = useRef<HTMLInputElement>(null);
    const [isUploading, setIsUploading] = useState(false);

    useEffect(() => {
        fetchDealAttachments(workspaceId, dealId);
    }, [workspaceId, dealId, fetchDealAttachments]);
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
            ...
            setIsUploading(true);
            try {
                await uploadDealAttachment(workspaceId, dealId, file);
            } catch (error) {
    ...
    const handleDelete = async (attachmentId: string) => {
        if (confirm('Are you sure you want to delete this attachment?')) {
            await deleteDealAttachment(workspaceId, dealId, attachmentId);
        }
    };
